...
export async function callApi(token: string, secret: string, call: string): Promise<object|null> {
    ...
    const escapedData = rawData.replace(toEscape, c => `\\u${('0000' + c.charCodeAt(0).toString(16)).slice(-4)}`)

    const timestamp = Math.round(new Date().getTime() / 1000)

    const hmacparameters = `${escapedData},${token},${query.actionid},${query.identifier},${query.resourceid},${secret},${timestamp},${query.resourcetype}`
